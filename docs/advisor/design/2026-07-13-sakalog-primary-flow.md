...
以下はConsolidated Findingsを実装可能な単位へまとめた候補であり、Findingを機械的に1件1Issueへ対応させない。

| 状態 | Issue化候補 | 対象CF | 想定スコープ | 備考 |
|---|---|---|---|---|
| #341 起票済み | 今日起点の主要閲覧ジャーニー再設計 | CF-001、CF-004 | トップ情報階層、重複ナビの役割、日付／公演文脈、詳細着地、戻り導線 | Product Purposeへ直接関わるため、実装前にIssue Decisionが必要。 |
