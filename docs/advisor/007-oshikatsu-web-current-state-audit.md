...
## Issue 化状況（2026-07-07 起票、同日すべて対応完了）

| Issue | 状態 | 対応項目 |
|-------|------|----------|
| #303 | ✅ Closed（PR #308） | SpotForm をセクション単位に分割する（728 → 337 行） |
| #304 | ✅ Closed（PR #309） | event create をトランザクション RPC 化し補償削除方式を廃止する（migration 060） |
| #305 | ✅ Closed（PR #310） | revalidateOrbit のタグ依存を宣言的な依存表に再構成する |
| #306 | ✅ Closed（PR #311） | readOrbitData.ts をドメイン別に分割する |

本監査の指摘は全件解消済み。次回監査の基準線はこの状態となる。
