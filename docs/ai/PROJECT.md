...
- CI：GitHub Actions（各アプリ配下の変更時に typecheck / lint / unit test（純関数・Vitest）/ build。oshikatsu-web は component test（jsdom・Testing Library）と migration の db-verify も実行）
...
- **楽曲/リリース管理**（リリース中心モデル、クレジット、参加メンバー、センター、フォーメーション、MV、関連動画、衣装）
  - 楽曲は「参加メンバーのみ」「参加メンバー＋センター」「完全なフォーメーション」を段階的に登録でき、フォーメーション未解禁でも参加メンバーとセンターを保持できる（Issue #427）
- **ライブ/会場/セットリスト管理**（公演、出演メンバー、披露メンバー、披露タイプ）
  - セットリストの楽曲項目は登録済みの `orbit_tracks` 選択を必須とする。`song_title` はread互換のためDB列に残すが、write側では使用しない（Issue #422 / PR #428）
  - フォーメーション入力は楽曲管理と同じ操作モデル（列人数 / 割当人数 / 候補選択 / ドラッグ・キーボード並べ替え）。割当候補は披露メンバーに限り、センターは披露メンバー欄から指定する（Issue #423）
  - 「オリメン」で楽曲参加メンバーを一括反映できる。反映対象は オリメン ∩ 公演の出演可能メンバー（ロスター − 休演）で、公演日時点の未加入・卒業も除外し、件数と理由を項目内へ表示する（Issue #424）
...
- 楽曲参加メンバーは、その楽曲の初出リリース（最も古い非nullのリリース日を持つ紐づきリリース。同日はリリースIDでタイブレーク）の参加メンバーから選ぶ。アルバム・再録など後続リリースの参加者は候補に含めない。不変条件は `楽曲参加メンバー ⊆ 初出リリース参加メンバー`（完全一致は要求しない）（ADR 0007の2026-07-26再改訂 / Issue #427）
- 初出リリースはDBへ明示保持せず、`pickFirstDatedRelease`で都度導出する。不変条件は楽曲のcreate/updateに加え、リリースのcreate/update/deleteでもDB由来factを使ってmutation前に強制する。新規リリースと既存の最古日が同じ場合だけ、ID未確定のため同日の全候補へ部分集合条件を要求する（ADR 0007の2026-07-28追記 / Decision #432 / Issue #448）
...
### セットリスト編集の運用メモ
- フォーメーション列の編集UIと列操作は `components/admin/formation/FormationRowsEditor.tsx` / `lib/formationRows.ts` を楽曲管理と共有する（Issue #423）
- 保存境界（`validateSetlist`）で「列人数 == 割当人数」「フォーメーションがある場合は配置集合 == 披露メンバー集合」「センターは披露メンバー内の最大2人、フォーメーションがある場合は1列目」を検証する
- 「オリメン」反映の判定は Server Action 内で完結させ、在籍履歴・ロスター・休演・公演日はすべてDBから取得し直す。クライアントから分類を受け取らない（Issue #424）
- 除外理由は 未加入 → 卒業 → 休演 → ロスター外 の優先順位で1つだけ割り当て、件数の合計が重複しないようにする
- 反映は非同期のため、対象項目の楽曲変更・手動編集は進行中の操作をキャンセルする。保存と反映は相互に排他とし、requestId が一致しない応答は常に破棄する（`usecases/originalMemberOperation.ts`）
...
### 今後の予定
`docs/orbit-roadmap.md` を参照。主要な設計判断は ADR 0005〜0016。
...
| 0007 | Orbit 選抜ポジションと参加メンバー・フォーメーションの正典 | Accepted |
...
| 0014 | テスト基盤としての Vitest 導入とテスト対象範囲 | Accepted |
...
- ~~テストは対象外~~: リファクタの安全性を機械的検証に頼ってきたが、ロジック退行を
  継続的に検知するため Phase 4 でテスト基盤（Vitest）を導入した（Issue #323 / ADR 0014）。
  #442 の再評価後、Sakalogでは操作密度の高いClient Componentに限り、jsdom・Testing Libraryの
  component testも追加した（Issue #450）。規約は `rules/implementation.md`「テスト」節を正とする
