...
- 楽曲参加メンバーは、その楽曲の初出リリース（最も古い非nullのリリース日を持つ紐づきリリース。同日はリリースIDでタイブレーク）の参加メンバーから選ぶ。アルバム・再録など後続リリースの参加者は候補に含めず、不一致は許容しない（ADR 0007の2026-07-26再改訂 / Issue #427）
- この不変条件はアプリケーション層で強制する。DBのハード制約は #426 で撤去した状態を維持し、初出リリースをDB上の明示的な関連として保持したあとに再導入する
