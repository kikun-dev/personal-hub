...
| ~~`setlist-center-toggle` specがnarrow viewportで間欠失敗する~~ | ~~light の 320px / 390px で `resolveBackgroundStack` が「祖先がすべて透明です」で throw する。失敗する viewport は実行ごとに移動する（320pxのみ → 320px+390px → 320pxのみ）~~ | ✅ Issue #465 で対応済み。原因は Chromium が computed custom property を `body` / `main` だけ空に報告するバグで、`html` とより深い子孫は正しい値を持つ（＝描画ではなく報告が壊れている）。祖先スタックの最外郭へ `:root` の `--background` を積んで解消した。詳細は [`apps/oshikatsu-web/playwright/README.md`](../apps/oshikatsu-web/playwright/README.md) の「既知の flaky test（最終状態）」 |
