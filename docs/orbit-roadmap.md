...
- [ ] 既存 `orbit_songs` 系テーブルを置換し、リリース中心モデルへ再設計する
- [ ] リリースタイプ `single/album/digital_single/other` とタイプ別ナンバリング制約を実装する
- [ ] 楽曲とリリースを M:N で管理し、リリースごとの曲順を保持する
- [ ] 人物マスタ新設 + クレジット（作詞/作曲/編曲/振付）複数人対応を実装する
- [ ] フォーメーション（列数・列ごとの人数・参加メンバー）と整合バリデーションを実装する
- [ ] MV（リンク必須条件付き）と衣装（複数 + 画像 + 担当）を実装する
- [ ] Admin CRUD と公開一覧/詳細を新モデルへ置換する
- [ ] メンバー詳細ページに参加楽曲セクションを追加する
