...
- [ ] 初出リリースをDB上の明示的な関連として保持し、楽曲参加メンバーの不変条件を保存境界で強制する（Issue #432）
- #425 の実装系列（#426 → #427 → #423 → #424）は完了。残るのは #432 のみ
...
- リリース日・収録曲・リリース参加メンバーの変更による事後的な不一致は現時点で防げない。既知の暫定状態として #432 で解消する
