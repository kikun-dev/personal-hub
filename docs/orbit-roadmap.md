...
- [x] A: 昼夜ラベル削除・単発/ツアー format・休演候補のロスター限定（Issue #109）
  - [x] `orbit_lives.format`（single/tour）追加、`session_label` 削除、RPC 更新。単発は1会場バリデーション
- [ ] C: 事前情報/当日カード横スライドのレイアウト再設計（Issue #111）
- [ ] B: 出演メンバー自動計算・会場/楽曲コンボボックス（Issue #110）
