...
- [x] 選抜/アンダー/期別、列、センターを楽曲ラベルとフォーメーションから導出し、リリース×メンバーの手動保持を福神/休業中 overlay に限定した（ADR 0007初版 / PR #180）。センターの正典は Issue #426 / PR #430 で `orbit_track_members.is_center` へ移行済み
...
### 楽曲参加メンバーとライブ初期値連携（#425 / ADR 0007改訂）

- [x] 楽曲参加メンバー、センター、任意のフォーメーションの正典と整合条件をDecision・ADRへ記録する（Issue #425）
- [x] `orbit_track_members` を追加し、既存フォーメーション・センターのbackfill、原子的な保存、メンバー詳細の参照先移行を行う（Issue #426 / PR #430）
- [x] 楽曲管理で「参加メンバーのみ」「参加メンバー＋センター」「完全なフォーメーション」を段階登録し、公開表示へ反映する（Issue #427 / PR #431・#433）
- [x] セットリストのフォーメーション入力UIを楽曲管理と同じ操作モデルへ揃える（Issue #423 / PR #435・#436）
- [x] 楽曲参加メンバーをコピーする「オリメン」操作と、卒業・休演の除外通知、現役ロスター未登録への登録導線を追加する（Issue #424 / PR #437）
- [x] 初出リリースはDBへ明示保持せず、TypeScriptでの都度導出を維持したまま、リリース側の全保存境界でも楽曲参加メンバーの不変条件を強制する（Decision #432 / Issue #448）
- #425 の実装系列（#426 → #427 → #423 → #424 → #448）は完了
- 列カードの編集UIは `components/admin/formation/FormationRowsEditor.tsx` と `lib/formationRows.ts` を楽曲管理とセットリストで共有し、2画面の再乖離を防ぐ（#423）
- 「オリメン」反映は Server Action で判定を完結させ、非同期中の競合は `usecases/originalMemberOperation.ts` の操作モデルで扱う（#424）
- リリース日・収録曲・リリース参加メンバーの変更とリリース削除による事後的な不一致は、create/update/delete UseCaseの保存前検証で防ぐ（#448）
...
- [x] C: セットリスト基盤（楽曲/MC/影アナ/VTR）（Issue #101）
  ...
  - [x] 当初は楽曲を登録曲（`orbit_tracks`）参照または未登録曲テキストとして実装。MC/影アナ/VTR/その他に対応
  - [x] 楽曲項目を登録曲参照必須へ変更し、未登録曲テキストのwriteを廃止。`song_title` 列とread表示は互換のため維持（Issue #422 / PR #428）
...
主要機能が概ね揃ったため、以降は「使いながら育てる」フェーズに移行する。
これまで「個人アプリだから許容」としてきた運用リスク（本番DB直あて・バックアップ手段の不在・
テスト不在・監視不在）を、この Phase で順に解消する。発端は 2026-07-07 の運用リスク棚卸し。

進める順序は P1（データ保全）→ P1.5（ローカル環境）→ P2（テスト・監視）とする。
上位ほど「失うと取り返しがつかない／下位の前提になる」ため優先度が高い。

- [x] **P1-1: バックアップ自動化 + リストア手順**（Issue #321）
...
- P3（テスト・環境が定着してから検討）:
  ...
  - ローカル環境を使った RLS ポリシーの自動テスト（認可の多層防御を機械的に検証する）
...
| `setlist-center-toggle` specがnarrow viewportで間欠失敗する | light の 320px / 390px で `resolveBackgroundStack` が「祖先がすべて透明です」で throw する。失敗する viewport は実行ごとに移動する（320pxのみ → 320px+390px → 320pxのみ）。ページは描画済みで `body` に `background: var(--background)` があるため本来は解決できるはずで、原因は未特定。#464 の検証中に `git stash` で main 由来と確認した | Issue #465 で対応。まず `ancestorBackgrounds` の実値をエラーへ含めて切り分け、計測タイミングか祖先チェーンかを特定してから修正方針を決める。未解決の間は #461 の残りPRでフル Playwright がこの1件だけ赤くなるため、本物の回帰と切り分ける必要がある |
...
なお上表の「foreground alphaによる文字・境界の組み立て」「Comboboxの `aria-controls`」は、この Full Design QA の総括とは別に、2026-07-30 のマージ後同期（#441 / #452 / #453 / #455 / #458）で記録した項目である。「`setlist-center-toggle` specがnarrow viewportで間欠失敗する」も同じく Full Design QA とは別で、#461 の PR2（#464）の検証中に発見して記録した。
