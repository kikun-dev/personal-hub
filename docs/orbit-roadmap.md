...
- [ ] セットリストのフォーメーション入力UIを楽曲管理と同じ操作モデルへ揃える（Issue #423）
- [ ] 楽曲参加メンバーをコピーする「オリメン」操作と、卒業・休演の除外通知、現役ロスター未登録への登録導線を追加する（Issue #424）
- [ ] 初出リリースをDB上の明示的な関連として保持し、楽曲参加メンバーの不変条件を保存境界で強制する（Issue #432）
- 実装順は `#426 → #427 → #423 / #424`。#423と#424は同じ `SetlistEditor` を変更するため、着手時に順序を調整する
- #427 時点では、リリース日・収録曲・リリース参加メンバーの変更による事後的な不一致を防げない。既知の暫定状態として #432 で解消する
