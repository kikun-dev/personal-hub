...
Vercel にデプロイしている `oshikatsu-web` の本番エラーに気づき、後から
原因を追えるようにするための最小運用手順。Issue #324 の Decision として、まずは新規依存を
最小限に抑え、Vercel の Runtime Logs / Observability / Web Analytics / Speed Insights を使う。

## 方針

- **採用するもの**: Vercel Runtime Logs / Observability / Web Analytics / Speed Insights / 標準通知
- **採用しないもの**: Sentry、APM、分散トレーシング、詳細なクライアント行動計測 / custom events
...
### 1. Web Analytics / Speed Insights を有効にする

`oshikatsu-web` の root layout に `@vercel/analytics` と `@vercel/speed-insights` の
Next.js App Router 用コンポーネントを配置している。

Vercel Dashboard → 対象 Project で次を有効にする。

- Analytics → Web Analytics
- Speed Insights

デプロイ後、`oshikatsu-web` にアクセスし、Dashboard に page view / Web Vitals が反映されることを
確認する。これらはエラー通知そのものではなく、利用状況・パフォーマンス劣化の把握に使う。
...
| `oshikatsu-web` | `app/layout.tsx` | Web Analytics / Speed Insights |
