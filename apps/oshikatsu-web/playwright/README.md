...
- 最終差分のfull suiteをfresh serverで1回: 205 pass / 9 skip / 0 fail
  （#453 で seed 041 を追加した後は **227 pass / 7 skip / 0 fail**。
  さらに #470 で seed 042 を追加した後は **233 pass / 5 skip / 0 fail**。内訳は下記）
...
各full suiteで発生する5件（seed 041 追加前は9件、seed 042 追加前は7件）のskipは、次の意図した条件と
一致した。想定外のskipはない。

- ~~管理フォームの編集2件: desktop/mobileのスポット各1件~~
  ...
  - **#470 でスポット2件がskipから外れた。** seed 042 が `orbit_spots` へ1件投入するため
    編集対象が存在するようになり、desktop/mobileとも実行されてpassする。
    これも一度も実行されていなかったケースで、#470 で実行してpassすることを確認した
    （231 pass / 7 skip → **233 pass / 5 skip**、いずれも 0 fail）
  - なお #470 の途中経過として、fixture に写真行を含めた版では両projectとも
    **失敗した**。Storage に実体が無いため `_next/image` が 400 を返し、本specの
    「リソース取得失敗ゼロ」assertに引っかかったため。fixtureが既存specを壊すのは
    本末転倒なので写真行は含めない判断にした（#470 の Decision に記録）
...
したがって5件（seed 041 追加前は9件、seed 042 追加前は7件）は環境・project条件による設計どおりの
skipで、flakyの隠蔽ではない。**スポットの編集hydrationは #470 の seed 042 で常時検証されるように
なった**（メンバー側は #453 の seed 041 で解消済み）。
