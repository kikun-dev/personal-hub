...
// 遷移先の状態を検証するtestは、操作前に取得したhrefから期待URLを組み立て、
// SPA遷移の完了を待ってからfocusを検証する（#418）。
async function resolveNonTodayDateLink(
  page: Page
): Promise<{ link: Locator; url: string }> {
  const link = page
    .getByRole("table")
  ...
  return { link, url: new URL(href, page.url()).toString() };
}
...
for (const viewport of interactionViewports) {
  ...
  test(`${viewport.width}pxで前月/翌月/date linkのhit areaが${viewport.minHitAreaPx}px以上`, async ({
    page,
  }) => {
    await page.setViewportSize(viewport);
    await page.goto("/");
...
test("MonthSelectorに「今日」ボタンを表示しない（ヘッダの今日導線と冗長なため撤去、#402）", async ({
  page,
}) => {
  await page.goto("/");

  await expect(page.getByRole("button", { name: "今日" })).toHaveCount(0);
});
...
// 「今日」ボタン撤去後（#402）、MonthSelectorの操作controlは「← 前月 / 翌月 →」の2つ。
// 上限（翌月 →）はShift+Tab先に「← 前月」が存在するが、下限（← 前月）はselector先頭のため
// Shift+Tab先はカレンダーのfocus contract外（上位コンテンツ）になる。下限では前方Tab順
// （翌月 →）のみをselector内契約として検証し、previousTargetNameは持たせない（null）。
const boundaryCases = [
  {
    label: "上限",
...
for (const boundary of boundaryCases) {
  test(`月範囲${boundary.label}でも押下buttonへfocusを維持しaria-disabled操作はno-op`, async ({
    ...
    if (boundary.previousTargetName) {
...
test("date選択後、見出しへfocusが移り、カレンダーへ戻るリンクで#calendarへ戻れる", async ({
  ...
  const { link: targetLink, url: dailyStoryUrl } =
    await resolveNonTodayDateLink(page);

  await targetLink.focus();
  await page.keyboard.press("Enter");
  await expect(page).toHaveURL(dailyStoryUrl);
  ...
  await page.keyboard.press("Tab");
  await expect(page.getByRole("button", { name: "← 前月" })).toBeFocused();
});
