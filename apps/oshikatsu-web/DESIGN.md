...
部品は親しみやすく軽快に扱える一方、形や状態表現は一貫させる。画面はモバイルからデスクトップまで最大幅 1280px の中心領域を基本とし（prose 主体の面は読みやすさのため page 側でより狭い読み幅に制限してよい）、カード一覧は 640px と 1024px を境に構造的に組み替える。動きは状態変化の説明に限り、装飾のための演出には使わない。
...
- **Error / Disabled:** エラーは境界をdanger（ライト `#B42318` / ダーク `#D92D20`）へ変え、danger text（ライト `#B42318` / ダーク `#FFB4AB`）の説明文を併用する。無効状態は不透明度とカーソルを変え、操作不能であることを視覚と挙動の両方で示す。
- **Form error banner:** フォーム全体のエラーは無彩色のsubtle面とsubtle境界の上にdanger textを置き、個々の項目エラーと役割を分ける。
