...
export function ReleaseDetail({ release }: ReleaseDetailProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <h2 className="mb-3 text-sm font-medium text-foreground-secondary">基本情報</h2>
        <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
          <dt className="text-foreground-secondary">リリースタイプ</dt>
          <dd className="text-foreground">{RELEASE_TYPE_LABELS[release.releaseType]}</dd>
          {release.releaseDate && (
            <>
              <dt className="text-foreground-secondary">リリース日</dt>
      ...
      {release.tracks.length > 0 && (
        <Card>
          <h2 className="mb-3 text-sm font-medium text-foreground-secondary">収録曲</h2>
          <ol className="space-y-2">
            {release.tracks.map((track) => {
              ...
              return (
                <li key={track.trackId} className="rounded-lg border border-border-subtle p-3">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="text-sm text-foreground-secondary">{track.trackNumber}.</span>
                    <Link
      ...
      {release.bonusVideos.length > 0 && (
        <Card>
          <h2 className="mb-3 text-sm font-medium text-foreground-secondary">特典映像</h2>
          <div className="space-y-4">
            {Array.from(bonusByEdition.entries()).map(([edition, items]) => (
              <section key={edition} className="space-y-2">
                <p className="text-xs font-medium text-foreground-secondary">{edition}</p>
                <ul className="space-y-2">
                  {items.map((bonus) => (
                    <li key={bonus.id} className="rounded-lg border border-border-subtle p-3 text-sm">
                      <p className="font-medium text-foreground">{bonus.title}</p>
                      {bonus.description && (
                        <p className="mt-1 whitespace-pre-wrap text-xs text-foreground-secondary">{bonus.description}</p>
                      )}
                    </li>
                  ))}
                </ul>
      ...
      {artworkSrc && (
        <Card>
          <h2 className="mb-3 text-sm font-medium text-foreground-secondary">収録曲アートワーク</h2>
          <Image
            src={artworkSrc}
            alt={`${release.title} artwork`}
            width={640}
            height={640}
            className="h-auto w-full rounded-lg border border-border-subtle object-cover"
          />
