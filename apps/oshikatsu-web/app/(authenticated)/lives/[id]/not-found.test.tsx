...
import LiveNotFound from "@/app/(authenticated)/lives/[id]/not-found";
...
describe("app/(authenticated)/lives/[id]/not-found.tsx", () => {
  it("ページを理解できるh1がちょうど1つ存在する", () => {
    render(<LiveNotFound />);

    expect(
      screen.getByRole("heading", { level: 1, name: "ライブが見つかりません" })
    ).toBeInTheDocument();
  });
  ...
  it("説明文が存在する", () => {
    ...
    expect(
      screen.getByText("指定されたライブは存在しないか、削除された可能性があります。")
    ).toBeInTheDocument();
  });
