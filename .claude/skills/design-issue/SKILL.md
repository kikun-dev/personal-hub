---
name: design-issue
description: プロジェクトのIssueテンプレートと設計提案フォーマットに沿ってGitHub Issueを起票する。「Issue化して」「Issueを作って」「設計Issueを起票」などで使う。feature/refactor/decisionの種別判定、ラベル付与、関連ドキュメント参照まで一括で行う。
...
- **refactor**: 背景（なぜ今やるか）/ スコープ（対象・非対象）/ リスク・影響範囲（影響・ロールバック）/ 完了条件 / Tasks
...
- 根拠になった調査・監査・ADR があれば背景からパスで参照する
  （例: `docs/advisor/006-oshikatsu-web-current-state-audit.md`、ADR 0006）
- 関連 Issue（前提・後続・同時実施が効率的なもの）があれば `#番号` で相互参照を書く
- 完了条件には検証方法を含める（typecheck / lint / 手動確認の観点。`rules/implementation.md` 参照）
- 本文は日本語ベース
...
作成した Issue が以下に該当する場合、対応表へ行を追加する:

- 技術的負債・既知の制限への対応 → `docs/orbit-roadmap.md` の「技術的負債・既知の制限」テーブル
- 監査・調査ドキュメント由来 → 元ドキュメント（`docs/advisor/`）の Issue 一覧
