...
### Codex のモデル・サブエージェント運用

- メインと成果物の統合・最終判断には Sol を使う
- ファイル探索、類似実装調査、テスト・E2E、証跡収集、定型変更など、独立して機械的に進められる作業は Terra / Luna へ優先して委譲する
- Astra は、難しい設計判断、原因不明の不具合、セキュリティ上重要な判断、大規模変更の最終レビューなど、高度な推論が必要な場合に限る。先に Sol / Terra / Luna で情報と論点を整理してから依頼する
- サブエージェントは独立性と並列化の効果がある場合に使い、必要以上に増やさない
- 確認済み事項の再調査と生ログの持ち込みを避け、長時間作業では継続に必要な決定・進捗・未解決事項をドキュメントへ整理する
